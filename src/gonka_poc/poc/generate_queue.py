            ...
            async for output in engine_client.generate(
                ...
                if poc_decode and max_tokens:
                    ...
                    if artifact["n_nan_steps"]:
                        # A non-finite step has no sphere id (the snap marks it
                        # -1): not work to commit, not a comparison to vote on.
                        # Dropping the nonce keeps -1 off the wire; on the
                        # validator side it leaves the sample (unscored_nonces).
                        logger.warning(
                            "PoC nonce %s: %d non-finite step(s) (compute fault) "
                            "— artifact dropped",
